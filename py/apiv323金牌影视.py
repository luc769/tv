...
修复版：首页兜底 + 详情不再批量请求播放页，解决首页刷不出、详情卡顿
...
class Spider(BaseSpider):
    ...
    def homeVideoContent(self):
        result = {"list": []}
        # 首页改为读取电影分类第一页兜底，解决首页页面结构失效
        html = self.getHtml(self.BASE_URL + "/channel/1.html")
        if not html:
            return result
    ...
    def _do_search(self, key, quick, pg="1"):
        ...
        if next_m:
            pagecount = str(page + 1)
        result["list"] = new_videos
        result["pagecount"] = pagecount
        result["total"] = str(int(pagecount) * len(videos)) if new_videos else "0"
        return result
